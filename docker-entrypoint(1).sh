...
if [ "$1" = 'mysqld' -a -z "$wantHelp" -a "$(id -u)" = '0' ]; then
	...
	file_env 'BACKUP_DOWNLOAD_URL'
	if [ -z "$MYSQL_ROOT_PASSWORD" -o -z "$BACKUP_DOWNLOAD_URL" ]; then
		echo >&2 'error: database is uninitialized and password option is not specified or backup download url is not specified.'
		echo >&2 '  You need to specify MYSQL_ROOT_PASSWORD and BACKUP_DOWNLOAD_URL'
		exit 1
	fi
	...
	mkdir -p $backup_dir
	echo "===========>downloading fullbackup from: $BACKUP_DOWNLOAD_URL/full.tar.gz ......"
	result=$(curl "$BACKUP_DOWNLOAD_URL/full.tar.gz" -o $backup_dir/full.tar.gz -w %{http_code} 2>/dev/null)
	if [ $result = 200 ];then
		...
		echo "===========>downloading incrementbackup from: $BACKUP_DOWNLOAD_URL/inc.tar.gz ......"
		result=$(curl "$BACKUP_DOWNLOAD_URL/inc.tar.gz" -o $backup_dir/inc.tar.gz -w %{http_code} 2>/dev/null)
		if [ $result = 200 ];then
			...
			echo "===========>increment[$BACKUP_DOWNLOAD_URL/inc.tar.gz] not exists."
		fi
		...
		echo >&2 "fullbackup[$BACKUP_DOWNLOAD_URL/full.tar.gz] not exists."
		exit 1
	fi
